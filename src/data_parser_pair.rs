...
impl SchemaParser<{Tag::Pair as u8}> for Pair {
    #[inline(always)]
    fn decode<'a>(schema: SchemaRef<'a>) -> SchemaEnum<'a> {
        ...
        // the rest of schema
        SchemaEnum::Pair(m, schema.slice(a..b), schema.slice(0..a))
    }
    ...
    fn dbg(schema: SchemaRef<'_>, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let SchemaEnum::Pair(_, a, b) = Self::decode(schema) else { unreachable!() };
        write!(fmt, "Pair({b:?}, {a:?})")
    }
